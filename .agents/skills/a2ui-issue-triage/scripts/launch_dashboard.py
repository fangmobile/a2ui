...
class TriageDashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_url.path == "/" or parsed_url.path == "/index.html":
            ...
            self.end_headers()
            html_path = os.path.join(os.path.dirname(__file__), "../assets/triage_dashboard.html")
            ...
            try:
                if os.path.exists(self.data_file):
                    ...
                    self.send_error_json(404, f"Triage comments data not found ({self.data_file}).")
...
def main():
    ...
    parser.add_argument("--data-file", required=True, help="Path to issues_to_triage.json.")
    parser.add_argument("--output-file", required=True, help="Path to save triage_decisions.json.")
    ...
    if not os.path.exists(resolved_data_file):
        print(f"Error: Data file '{resolved_data_file}' does not exist.", file=sys.stderr)
        sys.exit(1)
